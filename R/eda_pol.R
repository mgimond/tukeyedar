...
#'  median polish, but other statitcs such as the mean can be passed to the function
...
#'
#' eda_pol(df, row = "region", col = "edu")


eda_pol <- function (x, row= NULL, col = NULL, stat = median, plot = TRUE,
                        eps = 0.01, maxiter = 5,
  ...
  if(is.data.frame(x)){
    ...
    if(dim(x)[2] != 3) stop("The data frame must be in long form with 3 columns.")
    if(is.null(row)) stop("You need to define the row.name column.")
    if(is.null(col)) stop("You need to define the col.name column.")
  } else {
