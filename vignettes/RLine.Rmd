...
plot(eda_rline(neoplasms, Temp, Mortality), model=F, pt3=F, fit = F, show.par = F)
```
...
plot(eda_rline(neoplasms, Temp, Mortality), model=F, pt3=T, fit = F,show.par = F)
...
plot(eda_rline(neoplasms, Temp, Mortality,maxiter=1), model=F, pt3=T, 
     show.par = F, fit = F)
...
plot(eda_rline(df, Temp, Residual,maxiter=1), model=F, pt3=F, 
     show.par = F, fit = F)
#abline(v=brk.x,lty=3, col="grey")
...
plot(M, grey = 0.8, model = FALSE, show.par = F)
```
...
```{r fig.height=3.2, fig.width = 3.2, dev.args=list(pointsize=11)}
plot(M)
```


To see how this resistant line compares to an ordinary least-squares (OLS) regression slope, add the output of the `lm` model to the plot via the argument`reg = TRUE``:
...
```{r fig.height=3.2, fig.width = 3.2, echo=2, dev.args=list(pointsize=11)}
plot(M, reg = TRUE)
...
```{r fig.height=3.2, fig.width = 3.2,  results='hide', dev.args=list(pointsize=11)}
...
```{r fig.height=3.2, fig.width = 3.2,r, dev.args=list(pointsize=11)}
